...
async def test_download_skips_existing(tmp_path):
    route = respx.get("https://example.com/x.mp4").mock(return_value=Response(200, content=b"abc"))
    d = Downloader(cache_dir=tmp_path)
...
async def test_download_image_kind_uses_images_dir(tmp_path):
    respx.get("https://example.com/photo.jpg").mock(return_value=Response(200, content=b"img"))
    d = Downloader(cache_dir=tmp_path)
